...
class FlowDPMSolverMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # Runtime state initialized during sampling.
        ...
        if not use_dynamic_shifting:
            # Apply static sigma shifting unless dynamic shifting is enabled.
        ...
        # Keeping `sigmas` on device avoids extra host-device transfers here.
        self.sigma_min = self.sigmas[-1].item()
        self.sigma_max = self.sigmas[0].item()

    @property
    def step_index(self):
        """Current scheduler step index."""
        return self._step_index

    @property
    def begin_index(self):
        """Index of the first sampling step, if set by the caller."""
        return self._begin_index

    # Copied from diffusers.schedulers.scheduling_dpmsolver_multistep.DPMSolverMultistepScheduler.set_begin_index
    def set_begin_index(self, begin_index: int = 0):
        """Set the first sampling index before running inference.
    ...
    def index_for_timestep(self, timestep, schedule_timesteps=None):
        ...
        # Handle case where timestep is not found in schedule_timesteps
        if len(indices) == 0:
            # Find the closest timestep
            distances = torch.abs(schedule_timesteps - timestep)
